"""
Telegram-магазин в одном файле.
Python 3.11+
aiogram 3.x
SQLite
...
- Склад открывается постранично, поэтому работает даже при большом количестве товаров.
"""
...
SUBSCRIPTION_CHANNEL_URL = "https://t.me/owershop88"

# Тестовые 200 000 ₽ не учитываются только в общей рублёвой выручке.
EXCLUDED_TEST_REVENUE_RUB = 200_000
...
def calculate_custom_stars_price(product: sqlite3.Row, amount: int) -> int:
    """Цена пропорционально тарифу товара, с округлением вверх до целого рубля."""
    unit = product_unit_amount(product)
    return max(1, math.ceil(int(product["price_rub"]) * amount / unit))
            ...
            if product_id_info and int(product_id_info["notnull"]) == 1:
                ...
                self.conn.executescript(
                    ...
                    INSERT INTO orders_new (
                        ...
                        base_price_rub, base_price_stars, discount_percent, final_price_rub,
                        final_price_stars, promo_code, payment_method, stock_units, delivery_method,
                        receipt_file_id, receipt_type, status, created_at, processed_at, processed_by
                    )
                    SELECT
                        id, user_id, username, product_id, product_code, product_name, category,
                        base_price_rub, base_price_stars, discount_percent, final_price_rub,
            ...
            try:
                ...
                if product["category"] == "stars":
                    ...
                    if not is_allowed_gift_star_amount(units):
                        self.conn.rollback()
                        return False, "Недопустимое количество Stars.", None
                    if product["delivery_method"] != "gift":
                        self.conn.rollback()
                        return False, "Выдача Stars на аккаунт пока в разработке.", None
                ...
                cursor = self.conn.execute(
                    ...
                    (
                        ...
                        product["category"], base_price_rub, base_price_stars, discount,
                        final_price_rub, final_price_stars, normalized_promo, payment_method, units,
...
async def show_stars_delivery_choice(message: Message) -> None:
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🎁 Подарком", callback_data="stars:gift")],
            [InlineKeyboardButton(text="👤 На аккаунт", callback_data="stars:account")],
        ]
    )
    await message.answer(
...
async def ask_promo_for_product(message: Message, state: FSMContext, product: sqlite3.Row) -> None:
    ...
    if product["category"] == "stars":
        ...
        description = f"Вы выбрали <b>{stars(amount)}</b> подарком за <b>{money(price)}</b>."
    else:
...
async def show_payment_methods(message: Message, state: FSMContext) -> None:
    ...
    base_price_rub = (
        calculate_custom_stars_price(product, custom_units)
        if product["category"] == "stars"
        else int(product["price_rub"])
    )
    base_price_stars = int(product["price_stars"])
    final_price_rub = max(0, round(base_price_rub * (100 - discount) / 100))
    final_price_stars = max(0, round(base_price_stars * (100 - discount) / 100))
...
async def menu_stars(message: Message, state: FSMContext) -> None:
    await state.clear()
    await db.upsert_user(message)
    await show_stars_delivery_choice(message)
...
async def callback_stars_account(callback: CallbackQuery) -> None:
    await callback.answer("Раздел в разработке", show_alert=True)
    if callback.message:
        await callback.message.answer("👤 <b>Stars на аккаунт</b>\n\nРаздел в разработке.")
...
async def callback_stars_tariff(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if (
        ...
        await callback.answer("Stars по этому тарифу закончились.", show_alert=True)
        return
    await callback.answer()
    await state.clear()
...
async def process_stars_amount(message: Message, state: FSMContext) -> None:
    try:
        ...
        await message.answer("Введите количество Stars целым числом.")
        return
    data = await state.get_data()
    ...
    if not product or product["category"] != "stars" or product["delivery_method"] != "gift":
        await state.clear()
        await message.answer("Тариф больше недоступен. Начните покупку заново.")
        return
    available = product_available(product)
    if amount < 15:
        await message.answer("❌ Нельзя заказать меньше 15 Stars.")
        return
    if amount > available:
        await message.answer(f"❌ На складе доступно только {stars(available)}.")
        return
...
async def menu_premium(message: Message, state: FSMContext) -> None:
    await state.clear()
    await db.upsert_user(message)
    products = await db.get_products(category="premium")
    if products:
        await show_catalog(message, "premium")
    else:
        await message.answer(
            "💎 <b>Telegram Premium</b>\n\nРаздел находится в разработке. "
            "Когда владелец добавит товары категории <code>premium</code>, каталог включится автоматически."
        )
...
async def menu_reviews(message: Message) -> None:
    ...
    await message.answer("Отзывы покупателей находятся в отдельном Telegram-канале.", reply_markup=keyboard)
...
async def menu_support(message: Message) -> None:
    ...
    await message.answer("Нажмите кнопку ниже, чтобы открыть чат с поддержкой.", reply_markup=keyboard)
...
async def process_promo(message: Message, state: FSMContext) -> None:
    ...
    if not product:
        await state.clear()
        await message.answer("Товар больше недоступен. Начните покупку заново.")
        return
...
async def callback_payment_method(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if not product or product_available(product) < custom_units:
        await state.clear()
        await callback.answer("Товар закончился или остатка недостаточно.", show_alert=True)
        return
    ...
    if result == "approved" and order:
        ...
        try:
            await bot.send_message(
                ...
                "Администратор подтвердил оплату и свяжется с вами для ручной выдачи товара.",
            )
        except Exception:
...
async def command_admin(message: Message, state: FSMContext) -> None:
    if not message.from_user or not is_owner(message.from_user.id):
        ...
        return
    await state.clear()
    await message.answer("<b>⚙️ Админ-панель</b>", reply_markup=admin_menu_keyboard())
...
async def render_stock(message: Message, page: int = 0, edit: bool = True) -> None:
    ...
    rows.append([InlineKeyboardButton(text="➕ Добавить новый товар", callback_data="stock:new")])
...
async def process_new_promo(message: Message, state: FSMContext) -> None:
    ...
    parts = [part.strip() for part in message.text.split("|")]
    if len(parts) != 5:
        await message.answer("Нужно 5 полей: код | скидка | дата | лимит | категория")
        return
    ...
    if category is None:
        await message.answer("Категория: Все, Звезды, Аккаунты или Premium.")
        return
